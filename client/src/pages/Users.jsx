...
// ── User / Doctor modal ────────────────────────────────────────────────────
function UserModal({ editUser, hospitals, isDoctor, onSave, onClose, saving }) {
  const empty = {
    name: '', email: '', password: '',
...
export default function Users() {
  ...
  useEffect(() => {
    Promise.all([api.get('/api/users'), api.get('/api/hospitals')])
      .then(([u, h]) => { setUsers(u.data); setHospitals(h.data); })
    ...
    <>
      ...
      <main className="admin-main">
        ...
        <div className="admin-page-header">
          <div>
            <div className="admin-page-title">{isDoctor ? 'Doctors Users' : 'Users'}</div>
            <div className="admin-page-sub">{displayed.length} record{displayed.length !== 1 ? 's' : ''}</div>
          </div>
                    ...
                    <tr key={u._id}>
                      ...
                      <td>
                        <div className="action-btns">
                          <button className="btn-action edit"     title="Edit"            onClick={() => openEdit(u)}>✏️</button>
                          <button className="btn-action password" title="Change password" onClick={() => openPass(u)}>🔑</button>
                          <button className="btn-action lock"     title={u.locked ? 'Unlock' : 'Lock'} onClick={() => handleLock(u)}>
                            {u.locked ? '🔓' : '🔒'}
                          </button>
                          {u._id !== me?._id && (
                            <button className="btn-action delete" title="Delete" onClick={() => openDelete(u)}>🗑️</button>
                          )}
                        </div>
                      </td>
                    </tr>
                ...
                return (
                  <div className="user-card" key={u._id}>
                    ...
                    <div className="user-card-actions">
                      ...
                      {u._id !== me?._id && (
                        <button className="btn-action delete" onClick={() => openDelete(u)}>🗑️</button>
                      )}
                    </div>
                  </div>
                );
