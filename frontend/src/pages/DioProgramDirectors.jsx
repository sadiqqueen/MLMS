      ...
      <div style={{ background:'#fff', borderRadius:16, width:'100%', maxWidth:440, boxShadow:'0 20px 60px rgba(0,0,0,.2)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:14, padding:'20px 24px', borderBottom:'1px solid #E8E9EF' }}>
          <div style={{ width:48, height:48, borderRadius:'50%', background:'#1B1464', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, fontWeight:700, flexShrink:0 }}>
            {pd.initials || pd.name?.slice(0,2)?.toUpperCase() || '?'}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:17, fontWeight:700, color:'#1B1464' }}>{pd.name}</div>
            <div style={{ fontSize:12, color:'#8B8FA8', marginTop:2 }}>{pd.email}</div>
          </div>
        ...
        <div style={{ padding:'20px 24px' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px 20px' }}>
            {[
              ...
              ['Phone',      pd.phone      || '—'],
              ['City',       pd.city       || '—'],
              ['Hospital',   pd.hospitalId?.name || pd.hospital?.name || '—'],
              ['Status',     pd.isActive === false ? 'Inactive' : 'Active'],
            ].map(([label, value]) => (
              <div key={label}>
...
export default function DioProgramDirectors() {
  ...
  useEffect(() => {
    api.get('/api/dio/program-directors')
      ...
      .catch(() => showToast('Failed to load program directors'))
      ...
      <main className="admin-main">
        <div className="admin-card">
          ...
          <div className="admin-table-wrap">
            <table className="admin-table">
              <thead>
                <tr><th>#</th><th>Program Director</th><th>Department</th><th>Phone</th><th>Action</th></tr>
              </thead>
              <tbody>
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ textAlign:'center', padding:40, color:'#8B8FA8' }}>
                      ...
                      <div style={{ fontSize:15, fontWeight:600, color:'#4B5563' }}>
                        {pds.length === 0 ? 'No program directors in this hospital' : 'No program directors match your search'}
                      </div>
                    </td>
                  </tr>
                )}
                {filtered.map((p, i) => (
                  <tr key={p._id} style={{ cursor:'pointer' }} onClick={() => setSelected(p)}>
                    ...
                    <td>
                      <button className="btn-action edit" onClick={e => { e.stopPropagation(); setSelected(p); }}>View</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {selected && <PDModal pd={selected} onClose={() => setSelected(null)} />}
