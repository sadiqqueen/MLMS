...
//
// DIO / Sub-DIO oversight of the trainees in the caller's center set. Read-only
// table with search and a computed training-year badge (Y1–Y6).
// Contract: GET /api/dio-view/trainees[?search=] → trainees with injected
//   trainingYear, populated programId/hospitalId/specialtyId.
...
const STRINGS = {
  ar: {
    ...
    name: 'الاسم', idNumber: 'الرقم التعريفي', program: 'البرنامج', center: 'المركز', specialty: 'الاختصاص', year: 'السنة',
  ...
  en: {
    ...
    name: 'Name', idNumber: 'ID Number', program: 'Program', center: 'Center', specialty: 'Specialty', year: 'Year',
        ...
        <div className="admin-card">
          ...
          <div className="admin-table-wrap">
            <table className="admin-table"><tbody>
              {[...Array(8)].map((_, i) => (<tr key={i}><td><Sk w={130} h={13} /></td><td><Sk w={80} h={13} /></td><td><Sk w={120} h={13} /></td><td><Sk w={110} h={13} /></td><td><Sk w={90} h={13} /></td><td><Sk w={40} h={22} r={20} /></td></tr>))}
            </tbody></table>
          </div>
        </div>
      </main>
        ...
        <div className="admin-card">
          ...
          <div className="admin-table-wrap">
            <table className="admin-table">
              <thead>
                <tr><th>{t('name')}</th><th>{t('idNumber')}</th><th>{t('program')}</th><th>{t('center')}</th><th>{t('specialty')}</th><th>{t('year')}</th></tr>
              </thead>
              <tbody>
                {filtered.length === 0 && (
                  <tr><td colSpan={6} style={{ textAlign: 'center', padding: 32, color: 'var(--text-muted)' }}>{trainees.length === 0 ? t('none') : t('noMatch')}</td></tr>
                )}
                {filtered.map(tr => {
