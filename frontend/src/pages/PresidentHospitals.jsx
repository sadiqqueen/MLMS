  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.5)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
      ...
      <div style={{ background:'#fff', borderRadius:16, width:'100%', maxWidth:520, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,.2)' }}>
        {/* Header */}
        <div style={{ display:'flex', alignItems:'center', gap:14, padding:'20px 24px', borderBottom:'1px solid #E8E9EF', position:'sticky', top:0, background:'#fff', zIndex:10 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontSize:17, fontWeight:700, color:'#1B1464' }}>{hospital.name}</div>
            <div style={{ fontSize:12, color:'#8B8FA8', marginTop:2 }}>{hospital.city || hospital.governorate || '—'}</div>
          </div>
        ...
        <div style={{ padding:'20px 24px', display:'flex', flexDirection:'column', gap:18 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
            {[
              { label:'Trainees',   value: hospital.traineesCount  ?? 0, bg:'#DBEAFE', color:'#1E40AF' },
              { label:'Supervisors',value: hospital.supervisorsCount ?? 0, bg:'#D1FAE5', color:'#065F46' },
              { label:'Status',     value: hospital.isActive !== false ? 'Active' : 'Inactive', bg: hospital.isActive !== false ? '#D1FAE5':'#FEE2E2', color: hospital.isActive !== false ? '#065F46':'#991B1B' },
            ].map(s => (
          ...
          {Array.isArray(hospital.supervisors) && hospital.supervisors.length > 0 && (
            <div>
              <div style={{ fontSize:12, fontWeight:700, color:'#6B7280', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:8 }}>
                ...
                {filtered.map((h, i) => {
                  ...
                  return (
                    <tr key={h._id} style={{ cursor:'pointer' }} onClick={() => setSelected(h)}>
                      ...
                      <td>
                        <div style={{ fontWeight:700, color:'#1B1464' }}>{h.name}</div>
                        {h.address && <div style={{ fontSize:11, color:'#8B8FA8' }}>{h.address}</div>}
                      </td>
