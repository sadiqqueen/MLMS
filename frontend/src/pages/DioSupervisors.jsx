        ...
        <div style={{ padding:'20px 24px' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px 20px' }}>
            {[
              ...
              ['Status',      supervisor.isActive === false ? 'Inactive' : 'Active'],
            ].map(([label, value]) => (
              <div key={label}>
      ...
      <main className="admin-main">
        <div className="admin-card">
          ...
          <div className="admin-table-wrap">
            <table className="admin-table">
              <thead>
                <tr><th>#</th><th>Supervisor</th><th>Specialty</th><th>Department</th><th>Phone</th><th>Action</th></tr>
              </thead>
              <tbody>
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={6} style={{ textAlign:'center', padding:40, color:'#8B8FA8' }}>
                      ...
                      <div style={{ fontSize:15, fontWeight:600, color:'#4B5563' }}>
                        {supervisors.length === 0 ? 'No supervisors in this hospital' : 'No supervisors match your search'}
                      </div>
                    </td>
                  </tr>
                )}
                {filtered.map((s, i) => (
                  <tr key={s._id} style={{ cursor:'pointer' }} onClick={() => setSelected(s)}>
                    ...
                    <td>
                      <button className="btn-action edit" onClick={e => { e.stopPropagation(); setSelected(s); }}>View</button>
                    </td>
                  </tr>
                ))}
              </tbody>
