        ...
        <div className="admin-card">
              ...
              <tbody>
                ...
                {filtered.map((d, i) => (
                  <tr key={d._id} style={{ cursor:'pointer' }} onClick={() => setSelected(d)}>
                    ...
                    <td style={{ fontSize:13, color:'var(--text-2)' }}>{getHospital(d)}</td>
                    <td style={{ fontSize:13, color:'var(--text-2)' }}>{d.phone || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {view === 'card' && (
            <div className="management-card-grid">
              ...
              {filtered.map(d => (
                <div className="management-card" key={d._id} onClick={() => setSelected(d)} style={{ cursor:'pointer' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    ...
                    <div>
                      ...
                      <div className="management-card-sub">{d.email}</div>
                    </div>
                  </div>
                  <div className="management-card-sub">{getHospital(d)}</div>
                  <div className="management-card-sub">{d.phone || 'No phone'}</div>
                </div>
              ))}
            </div>
          )}
