...
export default function PresidentSupervisors() {
  ...
  const hospitalOptions  = [{ value:'', label:'All Hospitals' },   ...uniqueOptions(supervisors, getHospital)];
  const specialtyOptions = [{ value:'', label:'All Specialties' }, ...uniqueOptions(supervisors, getSpecialty)];

  const filtered = supervisors.filter(s => {
    if (hospitalFilter  && getHospital(s)  !== hospitalFilter)  return false;
    if (specialtyFilter && getSpecialty(s) !== specialtyFilter) return false;
    ...
    return !q
      ...
      || getHospital(s).toLowerCase().includes(q);
  });
...
<div className="admin-card">
          <div className="admin-toolbar" style={{ flexWrap:'wrap', gap:8 }}>
            <input className="admin-search" style={{ flex:1, minWidth:200 }} placeholder="Search by name, email, specialty, department, or hospital…" value={search} onChange={e => setSearch(e.target.value)} />
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={5} style={{ textAlign:'center', padding:40, color:'var(--text-muted)' }}>
                    ...
                    <div style={{ fontSize:15, fontWeight:600, color:'var(--text-2)' }}>
                      {supervisors.length === 0 ? 'No supervisors found' : 'No results match your search'}
                    </div>
                  </td></tr>
                )}
                {filtered.map((s, i) => (
                  <tr key={s._id} style={{ cursor:'pointer' }} onClick={() => setSelected(s)}>
                    ...
                    <td><span style={{ fontSize:11, fontWeight:600, padding:'3px 9px', borderRadius:20, background:'var(--chip-spec-bg)', color:'var(--chip-spec-fg)' }}>{getSpecialty(s)}</span></td>
          ...
          {view === 'card' && (
            <div className="management-card-grid">
              {filtered.length === 0 && <div className="admin-empty" style={{ gridColumn:'1/-1' }}>{supervisors.length === 0 ? 'No supervisors found' : 'No results match your search'}</div>}
              {filtered.map(s => (
                <div className="management-card" key={s._id} onClick={() => setSelected(s)} style={{ cursor:'pointer' }}>
                  ...
                  <div className="management-card-meta"><span style={{ fontSize:11, fontWeight:600, padding:'3px 9px', borderRadius:20, background:'var(--chip-spec-bg)', color:'var(--chip-spec-fg)' }}>{getSpecialty(s)}</span></div>
