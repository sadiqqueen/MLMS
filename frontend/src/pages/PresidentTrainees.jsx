...
function getSpecialty(t) { return t.specialtyId?.name || t.specialty || '—'; }
function getHospital(t)  { return t.hospitalId?.name  || t.hospital?.name || '—'; }
    ...
    <>
      ...
      <main className="admin-main">
        ...
        <div style={{ background:'#F0FDF4', border:'1px solid #BBF7D0', borderRadius:10, padding:'12px 16px', marginBottom:16, fontSize:13, color:'#166534', display:'flex', alignItems:'center', gap:8 }}>
          <span>👁</span> Read-only view — {trainees.length} trainee{trainees.length !== 1 ? 's' : ''} in this hospital
        </div>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={5} style={{ textAlign:'center', padding:40, color:'#8B8FA8' }}>
                    ...
                    <div style={{ fontSize:15, fontWeight:600, color:'#4B5563' }}>
                      {trainees.length === 0 ? 'No trainees in this hospital' : 'No results match your search'}
                    </div>
                  </td></tr>
                )}
                {filtered.map((t, i) => (
                  <tr key={t._id} style={{ cursor:'pointer' }} onClick={() => setSelected(t)}>
                    ...
                    <td>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        ...
                        <div><strong>{t.name}</strong><div style={{ fontSize:11, color:'#8B8FA8' }}>{t.email}</div></div>
                      </div>
                    </td>
                    <td><span style={{ fontSize:11, fontWeight:600, padding:'3px 9px', borderRadius:20, background:'#EEEDFE', color:'#3C3489' }}>{getSpecialty(t)}</span></td>
